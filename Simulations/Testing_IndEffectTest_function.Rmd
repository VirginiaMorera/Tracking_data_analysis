...
In this document I'll generate, using the package `SIMRiv`, simulated animal movement tracks to test the detection of Individual Site Fidelity by the function developed for the MS "Detecting recurrent sources of variability in animal tracking studies", by Virginia Morera-Pujol et al. (2020). The details of how the tracks with ISF can be simulated can be found [here](https://github.com/VirginiaMorera/Tracking_data_analysis) 
...
```{r sensitivitytest1 prepare,  results = 'hide', echo = TRUE, message = FALSE, warning = FALSE, fig.show = 'hide', cache = TRUE}
set.seed(123)
...
```{r sensitivitytest1 prepare2, results='hide', echo=TRUE, message=FALSE, warning=FALSE, fig.show='hide'}
...
Now we visualise the dataset we've created

```{r, results='hide', echo=TRUE, message=FALSE, warning=FALSE, fig.width=10.5, fig.height=7}
...
sdf <- sds %>% 
  ...
  separate(TrackID, into = c("BirdID", "TrackID"), sep = "_") %>%
...
```{r sensitivitytest1 run, message = FALSE, warning = FALSE, cache = TRUE}
...
Now we'll reproduce what the function does step by step. 

##### Obtain median, lower and upper bounds of CI  of the voerlap and name the matrices with the "BirdID"
...
BWint %>% 
  mutate(Contains = "NA") %>% 
  mutate(Contains = if_else(Low > WI_median, "Below", 
                              if_else(Low < WI_median & High > WI_median, "Within", 
                                      if_else(High < WI_median, "Above", Contains)))) -> BWint
...
sds %>% 
  st_as_sf(coords = c("coords.x1", "coords.x2")) %>% 
  separate(TrackID, into = c("BirdID", "TrackID"), sep = "_") -> sdf
