...
## ver3 
#   - add argument conditional from function kerneloverlap with defalut = T
#   - change argument name from inGroupVar to tripID
                          ...
                          nboots = 1000) #from Matching::ks.boot
{
  ...
  # plot boxplot
  if (plotIt) {
  print(ggplot(data = Overlaps, aes(x = Type, y = Overlap, fill = Type)) + geom_boxplot(notch = F) + theme_classic())
